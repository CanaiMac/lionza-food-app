...
# ============================================================
# CSS — Vista pública, cálida y apetitosa
# ============================================================
st.markdown("""
<style>
...
/* Ocultar sidebar y menú de Streamlit */
...
.stButton > button[kind="primary"]:hover {
    transform: translateY(-2px); box-shadow: 0 10px 30px rgba(212,160,23,0.4);
}
.stButton > button {
    background: #1e1e1e !important; color: #e8e2d9 !important;
...
.stTextInput input {
    ...
    border-radius: 10px !important; color: #e8e2d9 !important;
    font-family: 'DM Sans', sans-serif; font-size: 15px; padding: 12px !important;
}
...
.menu-card-emoji {
    ...
    background: #111; font-size: 38px; border-radius: 16px 16px 0 0;
}
...
.ticket-head {
    background: #222; padding: 14px 18px; border-bottom: 1px dashed #333;
...
.ticket-total {
    background: #222; padding: 14px 18px; border-top: 2px dashed #d4a017;
...
.cat-label {
    ...
    text-transform: uppercase; color: #555; margin: 20px 0 10px;
}
...
.hero-title {
    ...
    letter-spacing: 4px; color: #e8e2d9; line-height: 1;
}
.hero-title span { color: #d4a017; }
.hero-sub { font-size: 14px; color: #666; margin-top: 6px; letter-spacing: 1px; }
...
# ============================================================
# PERSISTENCIA DE PEDIDOS COMPARTIDA
# ============================================================
...
def guardar_pedido_cliente(pedido):
    pedidos = cargar_pedidos_pendientes()
...
# ============================================================
# CATÁLOGO (solo precios y nombres, sin inventario)
# ============================================================
PRECIOS_BASE = {
...
DESCRIPCIONES = {
    'Perro Caliente Simple':    'Pan + salchicha',
    'Perro Jumbo':              'Pan jumbo + salchicha',
    'Combo 6 Perros + 1.5L':   '6 perros + refresco 1.5L',
    'Combo Jumbos + 1.5L':     '4 jumbos + refresco 1.5L',
    'Hamburguesa Especial':     'Carne, huevo, jamón, queso',
    'Hamburguesa Sencilla':     'Carne, jamón, queso',
    'Medio Pollo Solo':         'Pollo + arepitas + ensalada',
    'Alitas x6':                '6 alitas + papas + arepitas',
    'Combo Pollo + 1.5L':       'Pollo + refresco + acompañantes',
    'Combo Pollo+2Perros+1.5L': 'Pollo + 2 perros + refresco',
...
# Tasa fija para mostrar precios en Bs (se actualiza desde el admin)
TASA_FILE = "tasa_dia.json"
def leer_tasa():
    if os.path.exists(TASA_FILE):
        try:
            return json.load(open(TASA_FILE))['tasa']
        except:
            pass
    return 419.05
...
if st.session_state.pedido_enviado:
    ...
    <div style="text-align:center; padding: 60px 20px;">
        ...
        <div style="font-family:'Bebas Neue',sans-serif; font-size:40px; color:#d4a017;
                    letter-spacing:3px; margin-top:16px;">¡PEDIDO RECIBIDO!</div>
...
st.markdown("""
<div class="hero">
    <div class="hero-title">🦁 LIONZA <span>FOOD</span></div>
    <div class="hero-sub">Haz tu pedido · Lo preparamos al momento</div>
</div>
""", unsafe_allow_html=True)
st.divider()
...
with col_n:
    nombre = st.text_input("👤 Tu nombre", placeholder="Ej: María González",
                           key=f"nombre_{st.session_state.key_reset}")
with col_h:
...
for cat_name, productos in CATEGORIAS.items():
    st.markdown(f'<div class="cat-label" style="margin-top:18px;color:#555;">{cat_name}</div>',
                unsafe_allow_html=True)
    ...
    for col, prod in zip(cols, productos):
        ...
        with col:
            ...
            if st.button("＋", key=f"add_{prod}_{st.session_state.key_reset}",
                ...
                for item in st.session_state.carrito:
                    if item['Producto'] == prod:
                        ...
                        item['BS']   = item['USD'] * TASA
                        existe = True
                        break
                if not existe:
                    st.session_state.carrito.append({
                        'Producto': prod, 'Cant': 1,
                        'USD': p_usd, 'BS': round(p_usd * TASA, 2)
                    })
...
if not st.session_state.carrito:
    ...
    <div style="background:#141414; border:1px dashed #2a2a2a; border-radius:14px;
                padding:32px 20px; text-align:center; color:#444;">
        <div style="font-size:32px;">🛒</div>
        <div style="font-size:13px; margin-top:8px;">Aún no has añadido nada</div>
    </div>
    """, unsafe_allow_html=True)
else:
    ...
    for item in st.session_state.carrito:
        items_html += (
            ...
            f'<span>{item["Cant"]}× {item["Producto"]}</span>'
            f'<span style="color:#d4a017;font-family:Bebas Neue;font-size:15px;">'
            f'${item["USD"]:.2f}</span></div>'
        )
    st.markdown(
        f'<div class="ticket">'
        f'<div class="ticket-head">Resumen de tu pedido</div>'
        + items_html +
        f'<div class="ticket-total">'
        f'<span style="font-size:12px;color:#888;letter-spacing:1px;text-transform:uppercase;">Total</span>'
        f'<div><span style="font-family:Bebas Neue;font-size:28px;color:#d4a017;">'
        f'${total_usd:.2f}</span>'
        f'<span style="font-size:11px;color:#666;margin-left:8px;">{total_bs:.0f} Bs.</span>'
        f'</div></div></div>',
        unsafe_allow_html=True
    )

    # Editar cantidades y eliminar
    st.markdown("<br>", unsafe_allow_html=True)
    for idx, item in enumerate(st.session_state.carrito):
        c1, c2, c3 = st.columns([4, 1, 1])
        c1.markdown(f"**{item['Producto']}**")
        nueva_cant = c2.number_input("", min_value=1, max_value=20,
                                      value=item['Cant'],
    ...
    nota = st.text_input("📝 ¿Alguna indicación especial? (opcional)",
                          ...
                          key=f"nota_{st.session_state.key_reset}")
    ...
    if st.button("🔥 ENVIAR PEDIDO", type="primary", use_container_width=True):
        if not nombre.strip():
            ...
            pedido = {
                ...
                'Nota':         nota.strip(),
                'Origen':       'cliente',  # marca para distinguir en admin
            }
            guardar_pedido_cliente(pedido)
            st.session_state.pedido_enviado = True
            st.rerun()

st.markdown("<br><br>", unsafe_allow_html=True)
st.markdown('<div style="text-align:center;font-size:11px;color:#333;">🦁 Lionza Food · Hecho con amor</div>',
            unsafe_allow_html=True)
